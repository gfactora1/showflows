...
export default function ShowDetail({ show, projectId, myRole, onBack }: Props) {
  ...
  const doSeed = async (existingAssignments: Assignment[]) => {
    ...
    const { error: insertErr } = await supabase
      .from('show_assignments')
      .upsert(inserts, { onConflict: 'show_id,person_id', ignoreDuplicates: true })
    ...
    const init = async () => {
      ...
      if (existing.length === 0 && canEdit) {
        ...
        try {
          ...
          // Suppress duplicate key errors — lineup already populated
  ...
  const manualSeed = async () => {
    ...
    setSeeding(true)
    try {
      // Clear all existing assignments first, then reseed from default roster
      ...
      const added = await doSeed([])
      await fetchAll()
      if (added === 0) {
        setMsg('Default roster is empty. Add people in Settings → Planning Defaults.')
      } else {
        setMsg(`Lineup reset to default roster (${added} person${added !== 1 ? 's' : ''}).`)
      }
    } catch (e: any) {
      setMsg(`Error resetting roster: ${e?.message ?? String(e)}`)
    } finally {
      setSeeding(false)
    }
  }
    ...
    <div>
        ...
        {canEdit && !seeding && (
          <button
            ...
            onMouseLeave={(e) => {
              ...
              e.currentTarget.style.color = '#B8C0D6'
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <button
              ...
              style={{
                fontFamily: 'inherit',
                opacity: loading || !addPersonId ? 0.5 : 1,
      ...
      {assignments.length === 0 && !seeding ? (
        <p style={{ opacity: 0.75 }}>
          No one assigned yet. Add people manually or reset to the default roster above.
        </p>
      ) : (
